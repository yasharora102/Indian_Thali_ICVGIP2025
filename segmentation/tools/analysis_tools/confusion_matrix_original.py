...
#                           color_theme='OrRd'):
#     """Draw confusion matrix with detailed styling using matplotlib.

#     Args:
#         confusion_matrix (ndarray): The confusion matrix.
#         labels (list[str]): List of class names.
#         save_dir (str|optional): If set, save the confusion matrix plot to the given path.
...
def plot_confusion_matrix(confusion_matrix,
                          ...
                          color_theme='OrRd'):
    """
    Safe normalization + annotation, no more -1% cells.
    """
    # --- 1) safe row-wise normalize to percentages ---
    # sum each row (ground truth)
    row_sums = confusion_matrix.sum(axis=1, keepdims=True)
    # divide only where the row_sum > 0, else leave the row as zeros
    norm_cm = np.divide(
        confusion_matrix.astype(np.float32),
        row_sums,
        out=np.zeros_like(confusion_matrix, dtype=np.float32),
    ...
    num_classes = len(labels)
    fig, ax = plt.subplots(
        figsize=(2 * num_classes, 2 * num_classes * 0.8), dpi=150)
    cmap = plt.get_cmap(color_theme)
    # im = ax.imshow(norm_cm, cmap=cmap, vmin=0, vmax=100)
    im = ax.imshow(norm_cm, cmap=cmap, origin='upper', vmin=0, vmax=100)
    cbar = plt.colorbar(im, ax=ax)
    cbar.ax.tick_params(labelsize=12)

    ax.set_title(title, fontdict={'weight':'bold', 'size':16})
    ax.set_ylabel('Ground Truth Label', fontdict={'size':14})
    ax.set_xlabel('Prediction Label', fontdict={'size':14})
    ...
    # --- 2) overlay the percentages, never -1% ---
    for i in range(num_classes):
        for j in range(num_classes):
            pct = norm_cm[i, j]
            txt = f"{pct:.1f}%"  if row_sums[i] > 0 else "0.0%"
            color = 'white' if pct > 50 else 'black'
            ax.text(j, i, txt, ha='center', va='center', color=color, fontsize=8)

    # fix y-axis invert bug matplotlib sometimes introduces
...
# def draw_simple_matrix(confusion_matrix,
#                        labels,
#                        save_dir=None,
#                        show=True,
#                        title="Simple Confusion Matrix",
#                        color_theme="viridis"):
#     """Draw a simple confusion matrix using matplotlib.

#     This is an alternative, less stylized visualization.
    
#     Args:
#         confusion_matrix (ndarray): The confusion matrix.
#         labels (list[str]): List of class names.
#         save_dir (str|optional): If set, save the plot to the given directory.
#         show (bool): Whether to show the plot immediately.
#         title (str): Title of the plot.
#         color_theme (str): Colormap to use (default: viridis).
#     """
#     fig, ax = plt.subplots()
#     im = ax.imshow(confusion_matrix, cmap=color_theme)
#     plt.title(title)
...
#     if save_dir is not None:
#         mkdir_or_exist(save_dir)
#         plt.savefig(os.path.join(save_dir, 'simple_confusion_matrix.png'), format='png')
#     if show:
#         plt.show()
...
#                        color_theme="viridis"):
#     """
#     Draw a clean, per-row normalized confusion matrix (0–100%) with annotations.
#     """
#     # 1) row-normalize to percentages
#     row_sums = confusion_matrix.sum(axis=1, keepdims=True)
#     # avoid div-by-zero: if a row has sum=0, leave it all zeros
#     norm_cm = np.divide(
#         confusion_matrix.astype(np.float32),
#         row_sums,
#         out=np.zeros_like(confusion_matrix, dtype=np.float32),
...
#     # 2) draw with fixed 0–100% scale
#     im = ax.imshow(norm_cm, cmap=color_theme, vmin=0, vmax=100)
...
#     if save_dir:
#         mkdir_or_exist(save_dir)
#         plt.savefig(os.path.join(save_dir, 'simple_confusion_matrix.png'), dpi=150)
#     if show:
#         plt.show()
...
def draw_simple_matrix(confusion_matrix,
                       ...
                       color_theme="viridis"):
    """
    Draw a clean, per-row-normalized 0–100% confusion matrix with annotations.
    """
    # 1) row-normalize to percentages
    row_sums = confusion_matrix.sum(axis=1, keepdims=True)
    norm_cm = np.divide(
    ...
    # 2) draw with fixed 0–100% scale
    im = ax.imshow(norm_cm, cmap=color_theme, origin='upper', vmin=0, vmax=100)
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label("Percent (%)")
    cbar.ax.yaxis.set_major_locator(MaxNLocator(integer=True))
...
def main():
    ...
    for img in sorted(os.listdir(args.prediction_path)):
        img_path = os.path.join(args.prediction_path, img)
    ...
    # Draw and save a simple version of the confusion matrix.
    draw_simple_matrix(
        confusion_matrix,
        dataset.METAINFO['classes'],
        save_dir=args.save_dir,
        ...
        title="Simple Confusion Matrix",
        color_theme="viridis")
    print('Simple confusion matrix plot saved.')
